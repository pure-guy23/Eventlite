...
public class Security extends WebSecurityConfigurerAdapter {
	...
	//add authentication constraint
	//add auth constraint for venues
	private static final RequestMatcher[] ADMIN = {new AntPathRequestMatcher("/events/new"), new AntPathRequestMatcher("/venues/new")};

	@Override
	protected void configure(HttpSecurity http) throws Exception {
		// By default, all requests are authenticated except our specific list.
		//edited to adapt the constraint of only administrator can add events
		http.authorizeRequests().requestMatchers(ADMIN).hasRole(ADMIN_ROLE).requestMatchers(NO_AUTH).permitAll().anyRequest().hasRole(ADMIN_ROLE);
