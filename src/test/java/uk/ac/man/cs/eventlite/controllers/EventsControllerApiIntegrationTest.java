...
//
//		client.delete().uri("/api/events/3").accept(MediaType.APPLICATION_JSON).exchange().expectStatus()
//				.isUnauthorized();
//
...
//	}
	...
	public void deleteEventBadUser() {
		...
		client.mutate().filter(basicAuthentication("Bad", "Person")).build().delete().uri("/api/events/2")
...
//		int currentRows = countRowsInTable("events");
//
//		client.mutate().filter(basicAuthentication("Rob", "Haines")).build().delete().uri("/api/events/4")
